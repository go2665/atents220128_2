...
public class SplittedItem : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        {
            itemIcon.sprite = itemSlot.SlotItemData.itemImage;  // itemSlot에 데이터가 있으면 슬롯에 있는 스프라이트를 사용하여 표시
        }
    ...
    private void OnClick(InputAction.CallbackContext context)
    {
        ...
        if (results.Count <= 0) // results.Count가 1개 이상이면 UI가 클릭된 것   
        {
            ...
            {
                GameObject obj = ItemFactory.GetItem(itemSlot.SlotItemData.id);                 // 아이템 슬롯에 들어있는 아이템 데이터를 이용해 아이템 생성
    ...
    public void Open(ItemSlot targetSlot, uint count)
    {
