...
public class BuyPanel : MonoBehaviour
{
    ...
    protected virtual void Awake()
    {
        ...
        baseDataLine = transform.Find("PlaceData").Find("BaseDataLine").GetComponent<BuyDataLine>();
    }
    ...
    /// <param name="cityBase">도착한 도시</param>
    public virtual void Show(bool isShow, Player arrived, CityBase cityBase)
    {
        if (isShow)
        {
            // UI를 보여줄 때
            ...
            placeName.text = cityBase.placeName;    // 도시 이름 설정
            baseDataLine.SetData(cityBase.price, cityBase.usePrice);    // 도시 가격 표시
            if (cityBase.CanBuy(arrived.Type))      // 구매 가능한지 확인
            {
                // 살 수 있으면 구매버튼 활성화
